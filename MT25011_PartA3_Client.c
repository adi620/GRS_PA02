...
void* client_thread(void *arg) {
    ...
    // Receive messages - zero-copy benefits are on sender side
    while (keep_running) {
...
int main(int argc, char *argv[]) {
    ...
    printf("[Client A3] Connecting to %s:%d\n", server_ip, server_port);
    printf("[Client A3] Message size: %d bytes\n", message_size);
    printf("[Client A3] Number of threads: %d\n", num_threads);
    printf("[Client A3] Duration: %d seconds\n", DURATION_SECONDS);
